            ...
            <div class="col-xl-8 mb-2">
                <div class="card">
                    <div wire:ignore class="card-body">
                        <div class="mb-3">
                            <label class="form-label">개인정보 처리방침</label>
                            <textarea wire:model="privacy" id="hugerte-mytextarea">{{ $privacy }}</textarea>
                        </div>
                    </div>
                </div>
                <div class="d-flex justify-content-end mt-2 mb-3">
                    <button id="updatePrivacy" wire:loading.attr="disabled" type="button" class="btn btn-primary">
                        업데이트
                    </button>
                </div>
            </div>
...
@section('js')
    ...
    <script>
        ...
        document.getElementById('updatePrivacy').addEventListener('click', function() {
            const editor = hugeRTE.get('hugerte-mytextarea');
            if (!editor) {
                console.error('Editor instance not found');
                return;
            }
            const privacy = editor.getContent();
